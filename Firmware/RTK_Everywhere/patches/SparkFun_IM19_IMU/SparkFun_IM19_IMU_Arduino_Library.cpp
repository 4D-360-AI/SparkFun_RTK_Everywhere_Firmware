...
// Testing shows that a the IM19 responds in approximate 2507ms.
// Try twice quickly here, but leave the retry logic to the host.
...
void IM19::eomHandler(IM19_PARSE_STATE *parse)
{
    ...
    else if (parse->messageType == im19MessageTypeMems)
    {
        IM19_CHECK_POINTER_VOID(packetMems, initMems);

        lastUpdateMems = millis();

        memcpy(&packetMems->data.timestamp, &parse->buffer[offsetMemsTimestamp], sizeof(double));
        memcpy(&packetMems->data.accelX,    &parse->buffer[offsetMemsAccelX],    sizeof(float));
        memcpy(&packetMems->data.accelY,    &parse->buffer[offsetMemsAccelY],    sizeof(float));
        memcpy(&packetMems->data.accelZ,    &parse->buffer[offsetMemsAccelZ],    sizeof(float));
...
bool IM19::initMems()
{
    packetMems = new IM19_MEMS_t;
    if (packetMems == nullptr)
    {
        debugPrintf("Pointer alloc fail");
        return (false);
    }
...
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.timestamp);
}
float IM19::getMemsAccelX()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.accelX);
}
float IM19::getMemsAccelY()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.accelY);
}
float IM19::getMemsAccelZ()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.accelZ);
}
float IM19::getMemsGyroX()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.gyroX);
}
float IM19::getMemsGyroY()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.gyroY);
}
float IM19::getMemsGyroZ()
{
    if (packetMems == nullptr) return (0);
    return (packetMems->data.gyroZ);
}
