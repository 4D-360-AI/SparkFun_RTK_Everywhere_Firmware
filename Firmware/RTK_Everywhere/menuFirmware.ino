// Generated by the Docker build (see Dockerfile). Guarded so a build without it
// compiles exactly as before.
#if __has_include("firmware_commit.h")
...
void firmwareVersionFormat(uint8_t major, uint8_t minor, char *buffer, int bufferLength, bool includeDate)
{
    ...
    // FIRMWARE_COMMIT is baked in by the Docker build. Appended rather than replacing the
    // version so nothing that parses the existing "v1.2" shape breaks, and so a build made
    // without it still produces exactly what it produced before.
    //
    // Without this, `rtkFirmwareVersion` reads "d99.99" on every developer build — the same
    // string for the build with the ephemeris fix and the build without it. A capture that
    // records that has recorded nothing.
#ifdef FIRMWARE_COMMIT
    if (buffer[0] && (int)(strlen(buffer) + strlen(FIRMWARE_COMMIT) + 2) <= bufferLength)
    {
        strncat(buffer, "+", bufferLength - strlen(buffer) - 1);
        strncat(buffer, FIRMWARE_COMMIT, bufferLength - strlen(buffer) - 1);
    }
#endif
...
void otaUpdateFirmware()
{
    ...
    if (response == ESP32OTAPull::UPDATE_AVAILABLE)
    {
        ...
        if (apConfigFirmwareUpdateInProcess)
            ...
            webServerSendString("confirmReset,1,");
        ESP.restart();
    }
